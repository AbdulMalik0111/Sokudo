...
const VerifyOtp = () => {
   ...
   const resendOtp = async () => {
      ...
      try {
         await fetch(`${process.env.NEXT_PUBLIC_API_URL}/customers/send-otp`, {
            method: 'POST',
   ...
   const onSubmit = async (data) => {
      ...
      try {
         const res = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL}/customers/verify-otp`,
            {
               method: 'POST',
               headers: { 'Content-Type': 'application/json' },
         ...
         <div className='w-full max-w-md bg-white border border-gray-200 p-8 rounded-2xl shadow-xl'>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className='space-y-5'>
               ...
               <div className='text-center text-sm'>
                  {!resendActive ? (
                     <span>
                        Resend OTP in <b>{counter}s</b>
                     </span>
                  ) : (
                     <button
                        type='button'
                        onClick={resendOtp}
                        className='text-[#ffb20e] font-semibold'
                     >
                        Resend OTP
                     </button>
                  )}
